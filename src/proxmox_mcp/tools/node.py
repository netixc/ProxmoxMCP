        ...
        try:
            ...
            for node in result:
                ...
                try:
                    ...
                    self.logger.warning(f"Cannot get detailed status for node {node_name}: {e}")
                    nodes.append({
                        "node": node_name,
                        "status": node["status"],
                        "uptime": "Requires Sys.Audit permission",
                        "maxcpu": "Requires Sys.Audit permission", 
                        "memory": {
                            "used": "Requires Sys.Audit permission",
                            "total": "Requires Sys.Audit permission"
                        }
                    })
